...
const CardNav = ({
  ...
  ease = 'power3.out',
  baseColor = '#fff',
  menuColor = '#fff', // default to white
  buttonBgColor,
  buttonTextColor
}) => {
  ...
  return (
    <div
      className={`card-nav-container w-[90%] max-w-[800px] z-99 ${className}`}
    >
      <nav
        ref={navRef}
        className={`card-nav blur-nav ${isExpanded ? 'open' : ''} block h-[60px] p-0 rounded-xl shadow-md relative overflow-hidden will-change-[height]`}
        // keep inline fallback but blur/background is handled by CSS .blur-nav
        style={{ backgroundColor: 'transparent', color: baseColor }}
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-4 pl-8 pr-8 z-2">
          <div
            ...
            tabIndex={0}
            style={{ color: menuColor }} // now defaults to white
          >
            <div
          ...
          {/* Logo (left) */}
          {logo && (
            <div className="nav-logo flex items-center gap-2 order-1 md:order-0">
