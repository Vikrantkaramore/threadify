...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '' }) => {
  const papers = items.slice(0, 4); // Limit to maximum 4 items but don't fill with null
  ...
  const getOpenTransform = index => {
    ...
    if (index === 3) return 'translate(70%, -70%) rotate(20deg)';
    return '';
  };
      ...
      >
        ...
        >
          ...
          {papers.map((item, i) => {
            ...
            if (i === 3) sizeClasses = open ? 'w-[100%] h-[80%]' : 'w-[100%] h-[50%]';
